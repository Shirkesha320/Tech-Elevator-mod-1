...
public class Exercises {
	...
	/*
	 2. We have two monkeys, a and b, and the parameters aSmile and bSmile indicate 
	 if each is smiling.
...
//	}
	...
	/*
	 5. We have a loud talking parrot. The "hour" parameter is the current hour time 
	...
	public boolean posNeg(int a, int b, boolean negative) {
			...
			if (negative) {
				...
				return (a < 0 && b >0) ||( a > 0 && b < 0); // one negative and one positive
			}
	}
...
//	}
	...
	/*
	 8. Return true if the given non-negative number is a multiple of 3 or a multiple 
	 of 5.
	 (Hint: Think "mod".)
	 or35(3) → true
	 or35(10) → true
	...
	/*
	 9. Given two temperatures, return true if one is less than 0 and the other 
	 is greater than 100.
	 icyHot(120, -1) → true
	 icyHot(-1, 120) → true
	 icyHot(2, 120) → false
	...
	public boolean loneTeen(int a, int b) {
		if (((a >= 13) && (a <= 19)) && ((b >= 13) && (b <= 19))) { // if both are teens then false
		return false;
	
		} else if (((a >= 13) && (a <= 19)) || ((b >= 13) && (b <= 19))) { // if either is a teen, then true
		return true; 
	...
	public int max1020(int a, int b) {
		// 2 = 1
		boolean aTenTwenty = a  >= 10 && a <= 20;
		
		if ((aTenTwenty) && (b >= 10 && b <= 20)) {  // both ints in range
			if (a > b)  {// if a is greater than b, return a
				return a;
		}
		 else {  // if both are in range and b is greater than a, return b
				return b;}
			 
		}
		else if ((a >= 10 && a <= 20) && (b < 10 || b > 20)) { // a only in range
				return a;
		}
			 
		 else if ((a < 10 || a > 20) && (b >= 10 && b <= 20)) { // b only in range
		 		return b;
		}
	...
	public boolean cigarParty(int cigars, boolean isWeekend) {
		// During the week the boolean can only be false not weekend
		
		if (((cigars >= 40) && (cigars <=60)) && ((!isWeekend))) { // successful during week
			return true;
		}
		
		else if (((cigars < 40) || (cigars > 60)) && ((!isWeekend))) { // unsuccessful during week
		   return false;
		}
		
		// During the weekend the boolean can only be true
		else if ((cigars >= 40) && (isWeekend )) { // successful during weekend
			return true;
		}
		
		else if ((cigars < 40) && (isWeekend )) {//unsuccessful during weekend
			return false;
		}
	...
	public boolean squirrelPlay(int temp, boolean isSummer) {
		if (((temp >= 60) && (temp <= 90)) && ((!isSummer))) { //60-90 range; not summer; squirrels play
		return true;
		
	}   if (((temp >= 60) && (temp <= 100)) && ((isSummer))) { //60-100 range; summer; squirrels play
		return true;
		
	}   if ((temp < 60 || temp > 90) && (!isSummer)) { // < 60 or > 90 not summer; squirrels not playing
		return false;
	
	}   if ((temp < 60) && (!isSummer)) { // < 60 not summer; squirrels not playing
		return false;
	     
	}   if ((temp > 100) && (isSummer)) { // > 100; summer; squirrels not playing
		return false;
	}
	    return false;
	}
    ...
    /*
     ...
     > $15 (ginormous). Unless it is your birthday -- on that day, an additional $5 is 
     added when calculating the category to potentially bump you up to the next 
     delicious category.

     For instance, if the amount of the meal was $7, and it is your birthday, $5 is added to
     $7 when determining the dessert category, which means you would qualify 
     for a "special".
	...
	public int sortaSum(int a, int b) {
		...
		if ((sum ) <= 9) { // Scenario 1 of 3: all sums totaling < 10
		return sum;
	
	}   else if (((a + b) >= 10) && ((a + b) <= 19)) {//Scenario 2 of 3: forbid range 10-19
		return 20;
	  	
	}   else if ((a + b) >= 20) {//Scenario 3 of 3: all sums 20 and over
		return sum;
	} return 20;
}
	/*
	...
	public String alarmClock(int day, boolean vacation) {
		if ((day >= 1 && day <= 5) && (!vacation)) { // weekday; no vacation
		return "7:00";

	}if ((day == 0 || day == 6) && (!vacation)) { // weekend; no vacation
		return "10:00";
		
	} if (vacation && day > 0 && day < 6) {// weekday; vacation
		return "10:00";
	
	} if (vacation && day == 0 || day == 6) {// weekend; vacation
	    return "off";
   }
	return "";
}
	...
	public boolean specialEleven(int n) {
		if (n % 11 == 0) { // Scenario 1: num is special; multiple of 11
			return true;
			
		} else if ((n % 11) == 1) { // Scenario 2: num is special; multiple of 11 + 1
			return true;

		} 		if ((n % 11) != 0){ // Scenario 3: num is not special; not a multiple of 11
			return false;
		}		
	...
	public boolean more20(int n) {
		{
			if (n % 20 == 1) { // Scenario 1: num is multiple of 20 + 1
				return true;
			}
			
			else if ((n % 20) == 2) { // Scenario 2: num is multiple of 20 + 2
				return true;
			}
	...
	public boolean old35(int n) {
		...
		if ((n % 3 == 0 || n % 5 == 0) && (!(n % 3 == 0 && n % 5 == 0))) {
			// First expression: n is either a multiple of 3 or 5
			// Second "": n cannot be a multiple of both 3 and 5
			return true;
		}		
	}
		return false;
	...
	public boolean less20(int n) {
		if ((n % 20 == 19)) { // Scenario 1: num is 1 < multiple of 20
			return true;
		}
		
		else if ((n % 20 == 18)) { // Scenario 2: num is 2 < multiple of 20
			return true;
		}
